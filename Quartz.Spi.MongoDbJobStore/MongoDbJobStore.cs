...
namespace Quartz.Spi.MongoDbJobStore
{
    public class MongoDbJobStore : IJobStore
    {
        ...
        public async Task StoreJob(IJobDetail newJob, bool replaceExisting, CancellationToken token = default(CancellationToken))
        {
            try
            {
                using (await _lockManager.AcquireLock(LockType.TriggerAccess, InstanceId))
        ...
        public async Task<bool> RemoveJobs(IReadOnlyCollection<JobKey> jobKeys, CancellationToken token = default(CancellationToken))
        {
            try
            {
                using (await _lockManager.AcquireLock(LockType.TriggerAccess, InstanceId))
        ...
        public async Task StoreTrigger(IOperableTrigger newTrigger, bool replaceExisting, CancellationToken cancellationToken = default(CancellationToken))
        {
            using (await _lockManager.AcquireLock(LockType.TriggerAccess, InstanceId))
            {
                await StoreTriggerInternal(newTrigger, null, replaceExisting, Models.TriggerState.Waiting, false, false, cancellationToken);
            }
        }

        public async Task<bool> RemoveTrigger(TriggerKey triggerKey, CancellationToken token = default(CancellationToken))
        {
            try
            {
                using (await _lockManager.AcquireLock(LockType.TriggerAccess, InstanceId))
        ...
        public async Task<bool> RemoveTriggers(IReadOnlyCollection<TriggerKey> triggerKeys, CancellationToken token = default(CancellationToken))
        {
            try
            {
                using (await _lockManager.AcquireLock(LockType.TriggerAccess, InstanceId))
                {
                    return triggerKeys.Aggregate(true, (current, triggerKey) => current && RemoveTriggerInternal(triggerKey).Result);
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> ReplaceTrigger(TriggerKey triggerKey, IOperableTrigger newTrigger, CancellationToken token = default(CancellationToken))
        {
            try
            {
                using (await _lockManager.AcquireLock(LockType.TriggerAccess, InstanceId))
        ...
        public async Task<IOperableTrigger> RetrieveTrigger(TriggerKey triggerKey, CancellationToken token = default(CancellationToken))
        {
        ...
        public async Task StoreCalendar(string name, ICalendar calendar, bool replaceExisting, bool updateTriggers, CancellationToken token = default(CancellationToken))
        {
            try
            {
                using (await _lockManager.AcquireLock(LockType.TriggerAccess, InstanceId))
        ...
        public async Task<ICalendar> RetrieveCalendar(string calName, CancellationToken token = default(CancellationToken))
        {
        ...
        public async Task<IReadOnlyCollection<JobKey>> GetJobKeys(GroupMatcher<JobKey> matcher, CancellationToken token = default(CancellationToken))
        {
            return (IReadOnlyCollection<JobKey>)new HashSet<JobKey>(await _jobDetailRepository.GetJobsKeys(matcher));
        }

        public async Task<IReadOnlyCollection<TriggerKey>> GetTriggerKeys(GroupMatcher<TriggerKey> matcher, CancellationToken token = default(CancellationToken))
        {
            return (IReadOnlyCollection<TriggerKey>)new HashSet<TriggerKey>(await _triggerRepository.GetTriggerKeys(matcher));
        }

        public async Task<IReadOnlyCollection<string>> GetJobGroupNames(CancellationToken token = default(CancellationToken))
        {
            return (IReadOnlyCollection<string>) await _jobDetailRepository.GetJobGroupNames();
        }

        public async Task<IReadOnlyCollection<string>> GetTriggerGroupNames(CancellationToken token = default(CancellationToken))
        {
            return (IReadOnlyCollection<string>) await _triggerRepository.GetTriggerGroupNames();
        }

        public async Task<IReadOnlyCollection<string>> GetCalendarNames(CancellationToken token = default(CancellationToken))
        {
            return (IReadOnlyCollection<string>) await _calendarRepository.GetCalendarNames();
        }

        public async Task<IReadOnlyCollection<IOperableTrigger>> GetTriggersForJob(JobKey jobKey, CancellationToken token = default(CancellationToken))
        {
        ...
        public async Task<TriggerState> GetTriggerState(TriggerKey triggerKey, CancellationToken token = default(CancellationToken))
        {
        ...
        public async Task<IReadOnlyCollection<string>> PauseTriggers(GroupMatcher<TriggerKey> matcher, CancellationToken token = default(CancellationToken))
        {
            try
            {
                using (await _lockManager.AcquireLock(LockType.TriggerAccess, InstanceId))
        ...
        public async Task<IReadOnlyCollection<string>> PauseJobs(GroupMatcher<JobKey> matcher, CancellationToken token = default(CancellationToken))
        {
            try
            {
                using (await _lockManager.AcquireLock(LockType.TriggerAccess, InstanceId))
        ...
        public async Task<IReadOnlyCollection<string>> ResumeTriggers(GroupMatcher<TriggerKey> matcher, CancellationToken token = default(CancellationToken))
        {
            try
            {
                using (await _lockManager.AcquireLock(LockType.TriggerAccess, InstanceId))
        ...
        public async Task<IReadOnlyCollection<string>> GetPausedTriggerGroups(CancellationToken cancellationToken = default(CancellationToken))
        {
            return (IReadOnlyCollection<string>)new HashSet<string>(await _pausedTriggerGroupRepository.GetPausedTriggerGroups());
        }
        ...
        public async Task<IReadOnlyCollection<IOperableTrigger>> AcquireNextTriggers(DateTimeOffset noLaterThan, int maxCount, TimeSpan timeWindow, CancellationToken token = default(CancellationToken))
        {
            try
            {
                using (await _lockManager.AcquireLock(LockType.TriggerAccess, InstanceId))
        ...
        public async Task ReleaseAcquiredTrigger(IOperableTrigger trigger, CancellationToken token = default(CancellationToken))
        {
            try
            {
                using (await _lockManager.AcquireLock(LockType.TriggerAccess, InstanceId))
        ...
        public async Task<IReadOnlyCollection<TriggerFiredResult>> TriggersFired(IReadOnlyCollection<IOperableTrigger> triggers, CancellationToken token = default(CancellationToken))
        {
            try
            {
                using (await _lockManager.AcquireLock(LockType.TriggerAccess, InstanceId))
        ...
        private async Task<IReadOnlyCollection<string>> PauseTriggerGroupInternal(GroupMatcher<TriggerKey> matcher, CancellationToken token = default(CancellationToken))
        {
            await _triggerRepository.UpdateTriggersStates(matcher, Models.TriggerState.Paused, Models.TriggerState.Acquired,
        ...
        private async Task PauseAllInternal()
        {
            ...
            await Task.WhenAll(groupNames.Select(groupName => PauseTriggerGroupInternal(GroupMatcher<TriggerKey>.GroupEquals(groupName))));
        ...
        private async Task ResumeTriggerInternal(TriggerKey triggerKey, CancellationToken cancellationToken = default(CancellationToken))
        {
            var trigger = await _triggerRepository.GetTrigger(triggerKey);
        ...
        {
            var groupNames = await _triggerRepository.GetTriggerGroupNames();
            await Task.WhenAll(groupNames.Select(groupName => ResumeTriggersInternal(GroupMatcher<TriggerKey>.GroupEquals(groupName))));
            await _pausedTriggerGroupRepository.DeletePausedTriggerGroup(AllGroupsPaused);
        }

        private async Task StoreCalendarInternal(string calName, ICalendar calendar, bool replaceExisting, bool updateTriggers, CancellationToken token = default(CancellationToken))
        {
            ...
            if (existingCal)
            {
                if (await _calendarRepository.UpdateCalendar(new Calendar(calName, calendar, InstanceName)) == 0)
                {
                ...
                if (updateTriggers)
                {
                    ...
                    foreach (var trigger in triggers)
                    {
                        ...
                        await StoreTriggerInternal(quartzTrigger, null, true, Models.TriggerState.Waiting, false, false, token);
                    }
                }
            }
            else
        ...
        private async Task<Models.TriggerState> CheckBlockedState(JobKey jobKey, Models.TriggerState currentState)
        {
            ...
            var firedTrigger = (await _firedTriggerRepository.GetFiredTriggers(jobKey)).FirstOrDefault();
        ...
        private async Task<TriggerFiredBundle> TriggerFiredInternal(IOperableTrigger trigger)
        {
            ...
            {
                calendar = (await _calendarRepository.GetCalendar(trigger.CalendarName))?.GetCalendar();
                if (calendar == null)
                {
                    return null;
                }
            ...
            Models.TriggerState newStateIfNotComplete, bool recovering)
        {
            ...
            {
                await StoreTriggerInternal(operableTrigger, null, true, Models.TriggerState.Complete, forceState, recovering);
        ...
        private async Task<IReadOnlyCollection<IOperableTrigger>> AcquireNextTriggersInternal(DateTimeOffset noLaterThan, int maxCount,
            TimeSpan timeWindow)
        {
        ...
        private async Task RecoverJobsInternal()
        {
            ...
            var results = (await _firedTriggerRepository.GetRecoverableFiredTriggers(InstanceId))
                .Select(async trigger => trigger.GetRecoveryTrigger(await _triggerRepository.GetTriggerJobDataMap(trigger.TriggerKey)));
            ...
            var completedTriggers = await _triggerRepository.GetTriggerKeys(Models.TriggerState.Complete);
